...
# Base MLP types
class MLP(nn.Module):
    def __init__(self, input_dim, hidden_dim, n_hidden_layers, output_dim):
...
class MyMLPResidualNetworkWithGloveEmbeddings(nn.Module):
    def __init__(
        ...
        embedding = nlp.make_embedding_layer_from_glove(
            glove_vectors, vocab, inverse_vocab, 300
        )
...
class MyMultiHeadAttention(nn.Module):
    def __init__(self, embedding_dim, n_attention_heads):
        ...
        self.mha = nn.MultiheadAttention(
            embedding_dim, n_attention_heads, batch_first=True
        )
...
class MyResidualNetworkWithMultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        pos_encodings = self.positional_encoding(
            torch.tensor(range(x.shape[1])).to(next(self.parameters()).device)
        )
        x = x + pos_encodings
        x = self.mha(x, x, x)
        x = self._pool(x)
